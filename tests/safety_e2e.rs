...
use zeroclaw::safety::snapshot::{SnapshotManager, SnapshotType, PathComponent, ComponentType, SnapshotMetadata};
use zeroclaw::safety::rollback::{
    TransactionManager, TransactionBuilder, RecoveryPolicy, 
    FileWriteOp, FileDeleteOp, RollbackData, ReversibleOp,
};
...
fn test_audit_hash_chain_integrity() {
    ...
    let logger = AuditLogger::new(config, dir.path().to_path_buf())
    ...
    let events = vec![
        ...
        AuditEvent::new(AuditEventType::ConfigChange)
            ...
            .with_action("update_settings".to_string(), "medium".to_string(), true, true),
    ];
...
fn test_audit_tamper_detection() {
    ...
    for i in 0..3 {
        let event = AuditEvent::new(AuditEventType::CommandExecution)
            .with_action(format!("command_{}", i), "low".to_string(), true, true);
        logger.log(&event).expect("Failed to log event");
    }
...
fn test_file_snapshot_and_restore() {
    ...
    // Create snapshot
    let snapshot = manager.create_snapshot(&test_file).expect("Failed to create snapshot");
    ...
    // Restore snapshot
    manager.restore_snapshot(&snapshot).expect("Failed to restore snapshot");
...
fn test_database_snapshot() {
    ...
    let snapshot = manager.create_database_snapshot(&db_path)
...
fn test_memory_snapshot() {
    ...
    let snapshot = manager.create_memory_snapshot(&memory_data, "session_state")
        .expect("Failed to create memory snapshot");
    
    assert_eq!(snapshot.snapshot_type, SnapshotType::Memory);
    
    // Restore memory snapshot
    let restored = manager.restore_memory_snapshot(&snapshot)
...
fn test_system_snapshot() {
    ...
    let snapshot = manager.create_system_snapshot(&components, metadata)
...
fn test_transaction_commit() {
    let dir = tempdir().expect("Failed to create temp dir");
    let store_path = dir.path().join("snapshots");
    let snapshot_manager = Arc::new(SnapshotManager::new(store_path).expect("Failed to create manager"));
    ...
    assert_eq!(std::fs::read(&test_file).expect("Failed to read file"), content);
}
...
fn test_transaction_rollback_on_failure() {
    ...
    // Verify new content
    assert_eq!(std::fs::read(&test_file).expect("Failed to read"), new_content);
...
fn test_recovery_policy() {
    ...
    assert_eq!(tx.status(), zeroclaw::safety::rollback::TransactionStatus::Pending);
}
...
fn test_full_safety_flow() {
    ...
    let audit_event = AuditEvent::new(AuditEventType::FileAccess)
        ...
        .with_resource_action("write".to_string(), target_file.to_string_lossy().to_string());
    
    audit_logger.log(&audit_event).expect("Failed to log audit");
    
    // 2. Create snapshot before modification
    let snapshot = snapshot_manager.create_snapshot(&target_file)
    ...
    audit_logger.log(&result_event).expect("Failed to log result");
    
    // 6. Verify audit chain
    let verification = audit_logger.verify_chain().expect("Failed to verify chain");
    assert!(verification.valid, "Audit chain should be valid after operations");
    assert_eq!(verification.total_events, 2);
    
    // 7. Cleanup (delete old snapshot)
    snapshot_manager.delete_snapshot(&snapshot).expect("Failed to cleanup snapshot");
    assert!(!snapshot.backup_path.exists());
}
...
fn test_safety_flow_with_rollback() {
    ...
    let start_event = AuditEvent::new(AuditEventType::ConfigChange)
        ...
        .with_resource_action("update".to_string(), target_file.to_string_lossy().to_string());
    audit_logger.log(&start_event).expect("Failed to log");
    
    // 2. Execute with transaction
    let tx = TransactionBuilder::new(snapshot_manager.clone())
        .write_file(target_file.clone(), br#"{"version": 2, "enabled": false}"#.to_vec())
        .build();
    ...
    let rollback_event = AuditEvent::new(AuditEventType::PolicyViolation)
        .with_agent("warden_001".to_string())
        .with_action("config_update_rollback".to_string(), "high".to_string(), false, false)
        .with_result(true, None, 25, Some("Policy violation: unsafe config change".to_string()));
    audit_logger.log(&rollback_event).expect("Failed to log rollback");
