    ...
    pub async fn recommendations(&self, stats: &UsageStats) -> Vec<OptimizationRecommendation> {
        ...
        if stats.cache_hit_rate < 0.3 {
            recs.push(OptimizationRecommendation {
                ...
                description: "Enable prompt caching to reduce API costs. Current cache hit rate is low.".to_string(),
        ...
        if let Some(openai_stats) = stats.by_provider.get("openai") {
            if openai_stats.cost_usd > stats.total_cost_usd * 0.5 {
                recs.push(OptimizationRecommendation {
                    ...
                    description: "Consider switching from OpenAI to DeepSeek for cost savings.".to_string(),
        ...
        if stats.avg_tokens_per_request > 2000.0 {
            recs.push(OptimizationRecommendation {
                ...
                description: "Average tokens per request is high. Consider prompt compression.".to_string(),
        ...
        if stats.request_count > 1000 {
            recs.push(OptimizationRecommendation {
                ...
                description: "High request count. Consider batching to reduce overhead.".to_string(),
