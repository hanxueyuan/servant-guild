...
impl BudgetState {
    ...
    fn hourly_spent(&self) -> f64 {
        self.hourly_spending.get(&self.current_hour).copied().unwrap_or(0.0)
    }
    
    /// Get current day spending
    fn daily_spent(&self) -> f64 {
        self.daily_spending.get(&self.current_day).copied().unwrap_or(0.0)
    }
...
impl BudgetManager {
    ...
    pub async fn spend(&self, amount: f64, agent: &str) {
        ...
        // Update counters
        *state.hourly_spending.entry(state.current_hour).or_insert(0.0) += amount;
        *state.daily_spending.entry(state.current_day.clone()).or_insert(0.0) += amount;
