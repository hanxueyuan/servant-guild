...
impl Guild {
    ...
    pub async fn with_config(config: GuildConfig) -> Result<Self, GuildError> {
        ...
        // Create servants
        let coordinator = Coordinator::new().with_consensus(consensus.clone());

        let worker = Worker::new().with_consensus(consensus.clone());

        let warden = Warden::new().with_consensus(consensus.clone());

        let speaker = Speaker::new(consensus.clone());

        let contractor = Contractor::new().with_consensus(consensus.clone());
    ...
    pub async fn process(&self, request: String) -> Result<GuildResult, GuildError> {
        ...
        *self.status.write() = GuildStatus::Processing;
        ...
        if let Some(audit) = &self.audit_log {
            ...
            let event = AuditEvent::new(crate::safety::AuditEventType::Custom(
    ...
    pub async fn execute_tool(
        ...
        let security_check = self
            .warden
            .read()
            .check_operation(tool_name, &params, "coordinator");
    ...
    pub async fn vote(
        ...
        self.consensus
            .cast_vote(proposal_id, voter_id.to_string(), vote, reason)
    ...
    pub fn is_healthy(&self) -> bool {
        ...
        statuses
            .values()
            .all(|s| *s == ServantStatus::Ready || *s == ServantStatus::Busy)
    }
}
...
mod tests {
    ...
    async fn test_guild_start_stop() {
        ...
        assert_eq!(
            statuses.get(&ServantRole::Coordinator),
            Some(&ServantStatus::Ready)
        );
        assert_eq!(
            statuses.get(&ServantRole::Worker),
            Some(&ServantStatus::Ready)
        );
