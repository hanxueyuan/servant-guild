# Contributing to ServantGuild

Thanks for your interest in contributing to ServantGuild! This guide will help you get started.

## First-Time Contributors

Welcome — contributions of all sizes are valued. If this is your first contribution, here is how to get started:

1. **Find an issue.** Look for issues labeled [`good first issue`](https://github.com/servant-guild-labs/servant-guild/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — these are scoped for newcomers and include context to get moving quickly.
...
4. **Start with Track A.** ServantGuild uses three [collaboration tracks](#collaboration-tracks-risk-based) (A/B/C) based on risk. First-time contributors should target **Track A** (docs, tests, chore) — these require lighter review and are the fastest path to a merged PR.
...
ServantGuild supports layered secret management for local development and CI hygiene.
...
2. **Config file** (`~/.servant-guild/config.toml`)
    - Persistent setup for long-term use
    - When `secrets.encrypt = true` (default), secret values are encrypted before save
    - Secret key is stored at `~/.servant-guild/.secret_key` with restricted permissions
    - Use `servant-guild onboard` for guided setup
...
- `~/.servant-guild/.secret_key` or similar key files
...
- If identity-like wording is required, use ServantGuild-centric labels only (for example: `ServantGuildAgent`, `ServantGuildOperator`, `servant-guild_user`).
...
- **Identity normalization**: when identity traits are unavoidable, use ServantGuild/project-native roles instead of personal or real-world identities.
...
ServantGuild's architecture is built on **traits** — every subsystem is swappable. This means contributing a new integration is as simple as implementing a trait and registering it in the factory function.
...
- **Identity-like labels**: if unavoidable, use ServantGuild-native identifiers only (`ServantGuildAgent`, `servant-guild_user`, `servant-guild_node`).
...
- **Good identity-like label**: `ServantGuildAgent`, `servant-guild_user`, `servant-guild_node`
...
- [ ] Any required identity-like wording uses ServantGuild/project-native labels only
